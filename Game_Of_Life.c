...
int main (int argc, char * argv[]) {
	...
	int T; 				//time steps
	int nr_of_threads;				//number of threads
	...
	if (argc != 4) {
		fprintf(stderr, "Usage: ./exec ArraySize TimeSteps NrOfThreads\n");
		exit(-1);
	}
	else {
		N = atoi(argv[1]);
	...
	for (t = 0 ; t < T ; t++) {
		#pragma omp parallel for collapse(2) schedule(static) private(i, j, nbrs)
		for (i = 1 ; i < N-1 ; i++)
			for (j = 1 ; j < N-1 ; j++) {
